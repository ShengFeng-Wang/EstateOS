...
## 2026-08-27 — Bade's 5 properties replaced with real named buildings; always-visible highlight added

**Spec/prior state:** Bade's 9 properties were procedurally generated like every other district (fictional name, fictional street + house number, per the demo-content rules in `estateos-responsive-localization-spec.md`).

**Change:** Two more direct, explicit follow-up requests, handled together: (1) "你要讓可以點的地方出現高亮或是有圖示表示這是我們管轄的大樓" — the clickable footprint now has an always-visible ground wash + ring highlight (colored by status) instead of only revealing itself on hover, so it's clear at a glance which real buildings are in the portfolio. (2) "你幫我直接選隨便5間的大樓 真實的...要是真實的喔 也根據真實座標位置" — Bade's procedurally-generated properties were replaced with 5 real, named buildings (城中大璽, 合遠首綻, 冠奕深耕13, 麗寶青年時代, 德億御邸) on their real roads, verified via web search against `community.houseprice.tw`'s Bade community listing.

**On pricing specifically:** the user's first instinct was to query a live rental listing site (591 or similar) for exact current asking prices. This was flagged as a real problem — those sites' terms of service generally prohibit scraping, and republishing a specific third party's real, currently-listed rental as if it belongs to this fictional company's managed portfolio would misrepresent a real person's real listing. Presented with this concern and alternatives via `AskUserQuestion`, the user chose the safe option: rent figures are instead grounded in real *public* data — 八德地政事務所's cited per-ping rent averages (via a news article) for relative price-tier ordering, scaled into this app's existing Apartment/Townhouse rent bands for consistency with the rest of the seeded portfolio. They are estimates cross-checked against real public statistics, not any specific unit's actual current asking rent.

**On coordinates:** the 5 buildings' real road is confirmed via search, but no geocoding tool was available to survey each building's exact plot — see `known-limitations.md` for how the coordinates were approximated within the correct real zone/street grid rather than surveyed to the literal building footprint.

**Why:** All explicit, direct requests. The scraping alternative was raised as a real ToS/misrepresentation concern rather than declined outright — the user chose the safer path once it was explained.
